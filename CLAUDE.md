...
1. **`docs/operasional-resto.md`** - ground truth alur bisnis resto (versi REV 2.11, sumber kebenaran tertinggi).
...
**Workflow order intake (REV 2.4):** waiter dan kasir **** untuk input order ke POS - keduanya mencatat pesanan via HP masing-masing. Waiter yang menghampiri meja bisa langsung input dari HP, atau menuliskan di kertas lalu diserahkan ke kasir untuk diinput - keduanya sama-sah (timing fleksibel, sistem tidak enforce). Yang tetap kewenangan kasir/owner: pembayaran, buka/tutup kasir, settlement (pemisahan tugas penanganan uang).
...
| **REV 2.11 COGS per Menu + Hapus Belanja/Vendor/Raw-Materials (sesi 2026-05-30)** | ✅ CODE-COMPLETE (branch `feat/cogs-per-menu-rev211`). Selaras balik ke proposal Bab 1 §1.4. **Tambah COGS**: per-menu owner-only `Menu.cost` (modal/COGS, TIDAK bocor ke GET publik POS) + `MenuCostMovement` change-log (reason initialSet/manualEdit) + `GET /menus/:id/cost-history` owner-only + `MenuVariant.costSourceMenuId` (sumber modal varian nonStock); snapshot `TransactionItem.unitCost` saat order via `resolveCostComponents` (kembar `resolveStockTargets`, sum modal komponen paket); owner dashboard **Laba Kotor = Pendapatan − COGS** (Σ unitCost×qty, filter identik revenue), **Tagihan (bills) ditampilkan terpisah, TIDAK dikurangkan** ke laba. **Hapus total** belanja/vendor/raw-materials (models `Vendor`/`Purchase`/`PurchaseItem`/`RawMaterial`/`RawMaterialMovement` + enum drop, modules `purchases`/`vendors`/`stocks/raw-materials` + UI Belanja/RawMaterialsTab + nav + reminder dihapus) → inventori = finished-goods porsi saja. Migrasi aditif dulu (`Menu.cost`/`costSourceMenuId`/`unitCost`/`MenuCostMovement` zero-loss), owner input modal, `backfill-cogs.ts` stamp unitCost historis, baru drop tabel belanja (DESTRUKTIF, PROD HARD-GATED). Docs ground-truth + knowledge (operasional-resto, ERD 14→10 entitas/23→17 relasi, USE-CASE 20→19 UC, ACTIVITY 11→9 diagram, DATA-DICTIONARY, FULL, BAB-3) diselaraskan. Spec: [docs/superpowers/specs/2026-05-30-cogs-per-menu-remove-belanja-design.md](docs/superpowers/specs/2026-05-30-cogs-per-menu-remove-belanja-design.md). Plan: [docs/superpowers/plans/2026-05-30-cogs-per-menu-remove-belanja.md](docs/superpowers/plans/2026-05-30-cogs-per-menu-remove-belanja.md). |
| StarUML UC | ✅ 3 aktor (Kasir/Waiter/Owner) + 20 use case + UMLConstraint REV 2.3 note (input order kasir+waiter) sudah lengkap di `Skripsi.mdj`. ⏳ REV 2.11 pending rebuild (19 UC: drop pembelian/opname-raw, +Kelola Modal/COGS). |
...
| `docs/knowledge/FULL.md` | ✅ REV 2.11 (overview kompilasi 3 design - 10 entitas/17 relasi, 19 UC, 9 activity) |
| `docs/knowledge/BAB-3-DRAFT.md` | ✅ REV 2.11 faktual diselaraskan (drop pembelian/raw + COGS FR) - ⚠️ **renumber Gambar/Tabel + kehalusan prosa naratif perlu review thesis-level Ezra** (di-flag di banner atas file). |
| `docs/DATA-DICTIONARY.md` | ✅ REV 2.11 (10 entitas + 17 relasi, +menu_cost_movements +cost/unit_cost). |
| `docs/knowledge/{ERD,USE-CASE,ACTIVITY}.md` | ✅ REV 2.11 (ERD 10 entitas/17 relasi, USE-CASE 19 UC, ACTIVITY 9 diagram). |
| `Skripsi.mdj` (StarUML) | ⏳ ERD + Activity Diagrams + UC **pending rebuild REV 2.11** (drop entitas belanja/raw + tambah `menu_cost_movements`; drop 2 activity; 19 UC). Visual deferred/opsional. |
...
- **Order intake (REV 2.4)**: input order **** kasir + waiter via HP. Dine-in (pilih meja) maupun takeaway bisa diinput oleh waiter langsung dari HP atau oleh kasir; workflow kertas jadi salah satu cara, bukan satu-satunya. Pembayaran tetap kasir/owner-only.
...
Routing di [frontend/src/App.tsx](frontend/src/App.tsx): `ProtectedRoute` gate by auth; `OwnerRoute` requires `role === 'owner'`. **Target REV 2.3**: tambah `CashierRoute` guard + 3 dashboard berbeda layout (`OwnerDashboard`, `CashierDashboard`, `WaiterDashboard`) - waiter dan kasir untuk input order (REV 2.4).
