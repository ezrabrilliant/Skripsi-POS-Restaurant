    ...
    <div className="h-full flex">
      ...
      <div className="w-1/2 border-r border-neutral-200 flex flex-col">
        ...
        <div className="p-4 bg-white border-b border-neutral-200">
          <h1 className="text-xl font-semibold text-neutral-800 mb-4">Riwayat Transaksi</h1>
          ...
          <div className="p-4 bg-neutral-50 border-b border-neutral-200">
            <div className="grid grid-cols-2 gap-2">
              <div className="bg-white p-3 rounded-lg">
                ...
                <p className="font-semibold text-neutral-800">{summary.totalTransactions}</p>
              </div>
              <div className="bg-white p-3 rounded-lg">
                <p className="text-xs text-neutral-500">Total Penjualan</p>
                <p className="font-semibold text-primary-600">{formatCurrency(summary.grandTotal)}</p>
              </div>
            </div>
          </div>
        )}
        ...
        <div className="flex-1 overflow-y-auto">
          {isLoading ? (
            <div className="p-4 space-y-3">
              {[...Array(5)].map((_, i) => (
                <div key={i} className="bg-neutral-200 h-20 rounded-lg animate-pulse" />
              ))}
            </div>
              ...
              {transactions.map((tx) => (
                <li key={tx.id}>
                  <button
                    ...
                    className={cn(
                      'w-full p-4 text-left hover:bg-neutral-50 transition-colors flex items-center justify-between',
                      selectedTransaction?.id === tx.id && 'bg-primary-50'
                    )}
                  >
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className="font-medium text-neutral-800">Meja {tx.tableNumber}</span>
        ...
        {selectedTransaction ? (
          <div className="h-full flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-4">
              {/* Items */}
              <h3 className="font-medium text-neutral-600 mb-2">Items</h3>
              <ul className="space-y-2 mb-6">
                {selectedTransaction.items.map((item) => (
                  <li key={item.id} className="flex justify-between">
                    ...
                    <span className="text-neutral-800">{formatCurrency(item.subtotal)}</span>
                  </li>
                ))}
              </ul>
